...
const FingerprintTable = () => {
  ...
  const handleDelete = async (id) => {
    const action = async () => {
      const res = await deleteEmployeeFingerprint(id, token);
      console.log(res);
      if (res.status === 204) {
    ...
    handleConfirm("bạn muốn xóa hình ảnh vân tay này?", action);
  };
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        const response = await getAllEmployeeFingerprint(employee_id, token);
            ...
            {filteredEmployees.map((employee) => (
              ...
              >
                <TableCell align="center">
                  <img
                    src={convertToImageUrl(employee.fingerprint)}
